<template>
    <div class="fullscreen flex">
        <video ref="videoPlayer" :src="videoSrc" :poster="posterSrc" @ended="onVideoEnd" @click="startVideo"
            playsinline></video>
        ...
        <div class="quiz-container" v-if="showQuiz">
            ...
            <div>
                <button v-for="(answer, index) in currentQuestion.answers" :key="index" class="answer-btn"
                    :class="getAnswerClass(index)" @click="handleAnswer(index)" :disabled="buttonsDisabled">
                    <div v-if="answer.image">
                        <img :src="answer.image" :alt="answer.text" style="width: 100px; height: 100px" />
                        <p>{{ answer.text }}</p>
                    </div>
                    <div v-else>
                        <p>{{ answer }}</p>
            ...
            <button v-if="feedbackVisible && !isLastQuestion" class="next-btn" @click="nextQuestion">
                Далі
            </button>
            <button v-if="feedbackVisible && isLastQuestion" class="next-btn" @click="finishQuiz">
                Завершити
            </button>
        </div>
    </div>
...
const handleAnswer = (index) => {
    ...
    if (isCorrect) {
        score.value++;
        playSound(soundCorrect.value);
        feedbackMessage.value = "✅ Правильно!";
    } else {
        playSound(soundWrong.value);
